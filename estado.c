...
int heuristica_manhattan(int matriz[3][3]) {
    ...
    for (int i = 0; i < 3; i++) {
        for (int j = 0; j < 3; j++) {
            ...
            if (valor != 0) {
                int linha_correta  = (valor - 1) / 3;
                int coluna_correta = (valor - 1) % 3;
                total += abs(i - linha_correta) + abs(j - coluna_correta);
            }
        }
    }
    return total;
...
Estado* criar_estado(Estado *pai, char movimento, int matriz_nova[3][3]) {
    ...
    novo_estado->g = (pai == NULL) ? 0 : pai->g + 1;
    novo_estado->h = heuristica_manhattan(novo_estado->matriz);
    novo_estado->f = novo_estado->g + novo_estado->h;

    novo_estado->anterior = pai;
    novo_estado->movimento = movimento;
    novo_estado->profundidade = (pai == NULL) ? 0 : pai->profundidade + 1;
